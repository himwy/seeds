...
const HOME_RECENT_EVENTS_PREVIEW = 3;

function HomeRecentEventCard({
  event,
  language,
  className = "",
}: {
    ...
    >
      <article className="flex h-full flex-col overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm transition-shadow duration-300 hover:shadow-md">
        ...
        <div className="flex flex-1 flex-col p-5 md:p-6">
          <time
            dateTime={event.date}
            className="text-sm font-semibold tracking-wide text-[var(--primary)]"
          >
            {formatted}
          </time>
          <h3 className="mt-2 line-clamp-2 text-lg font-bold leading-snug text-[var(--primary)]">
            {title}
          </h3>
          <span className="mt-auto inline-flex items-center gap-2 pt-4 text-sm font-medium text-dark-gray">
            {language === "en" ? "View event" : "查看活動"}
...
export default function Home() {
  ...
  if (isMobile) {
    return (
      <div className="w-full overflow-x-hidden">
        ...
        {/* Mobile Recent Events — static editorial list */}
        {recentEventsPreview.length > 0 && (
          <section className="relative overflow-hidden bg-light-gray py-16 md:hidden">
            <div className="relative z-10 px-6">
              <div className="mb-12 text-center">
                <h2 className="mb-4 text-3xl font-bold text-[var(--primary)]">
                  {language === "en" ? "Recent Events" : "最近活動"}
                </h2>
                <p className="mx-auto max-w-2xl text-dark-gray">
                  {language === "en"
                    ? "Stay updated with our latest professional activities and milestones."
                    : "了解我們最新的專業活動與里程碑。"}
                </p>
              </div>
  ...
  return (
    <div className="overflow-x-hidden w-full">
      ...
      {/* Recent Events — static grid (desktop) */}
      <section className="hidden w-full bg-light-gray py-12 md:block md:py-20">
        <div className="container mx-auto px-6">
          <div className="fade-in-up mb-10 text-center md:mb-16">
            <h2 className="mb-4 text-2xl font-bold text-[var(--primary)] md:text-3xl">
              {language === "en" ? "Recent Events" : "最近活動"}
            </h2>
